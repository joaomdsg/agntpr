...
// fundBandwidth gives a session's attention meter earned bandwidth by
// clearing one block — the same pattern live_place_order_internal_test.go
// uses to fund a live PlaceOrder.
...
func TestLiveCard_authorHandshakeWritesToTheProtectedDirectoryAndCachesIt(t *testing.T) {
	...
	log := ledger.Bind(f, "authorhs", "i")
	fundBandwidth(t, log) // the authoring control only renders with bandwidth to place with
	registerSession("authorhs", LiveConfig{RepoDir: repo, BaseRev: head, Anchor: anchorForCap(), TestCmd: []string{"true"}}, log)
	...
	require.Equal(t, 200, tc.Action((&LiveCard{Key: "authorhs"}).AuthorHandshake).
...
func TestLiveCard_authorHandshakeIsANoOpOnAnEmptyDraft(t *testing.T) {
	...
	require.NoError(t, err)
	t.Cleanup(func() { _ = f.Close() })
	log := ledger.Bind(f, "authorhsempty", "i")
	registerSession("authorhsempty", LiveConfig{RepoDir: repo, BaseRev: head, Anchor: anchorForCap(), TestCmd: []string{"true"}}, log)
	...
	require.Equal(t, 200, tc.Action((&LiveCard{Key: "authorhsempty"}).AuthorHandshake).
		WithSignal("handshakestrengthpick", "examples").
		Fire())
...
func TestLiveCard_authorHandshakeIsANoOpWithoutADeclaredStrength(t *testing.T) {
	...
	require.NoError(t, err)
	t.Cleanup(func() { _ = f.Close() })
	log := ledger.Bind(f, "authorhsnostrength", "i")
	registerSession("authorhsnostrength", LiveConfig{RepoDir: repo, BaseRev: head, Anchor: anchorForCap(), TestCmd: []string{"true"}}, log)
	...
	require.Equal(t, 200, tc.Action((&LiveCard{Key: "authorhsnostrength"}).AuthorHandshake).
		WithSignal("handshakedraft", "package handshake\n\nfunc TestSpec(t *testing.T) {}\n").
		Fire())
...
func TestLiveCard_rendersTheHandshakeAuthoringControl(t *testing.T) {
	...
	require.NoError(t, err)
	t.Cleanup(func() { _ = f.Close() })
	log := ledger.Bind(f, "handshakecontrol", "i")
	fundBandwidth(t, log)
	registerSession("handshakecontrol", LiveConfig{RepoDir: ".", BaseRev: "b", Anchor: anchorForCap(), TestCmd: []string{"true"}}, log)
